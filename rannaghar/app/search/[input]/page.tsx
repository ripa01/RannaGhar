import type { Meal } from "@/app/globals/types"
import { search } from "@/app/utils/search"
import Image from "next/image"
import Link from "next/link"

export default async function SingleCategory({ params }: { params: Promise<{ input: string }> }) {
  const { input } = await params
  const meals: Meal[] | null = await search(input)
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-6xl">
      ...
      <h1 className="text-3xl font-bold text-gray-800 mb-6">
        Search Results: <span className="text-teal-500">{decodedInput}</span>
      </h1>
